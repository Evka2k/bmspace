...
bms_version = 'STD1-16S100JC11-V1.2.2-T4'
bms_sn = 'sn1'
pack_sn = ''
packs = 4
cells = 16
...
#    cycles = []
    ...
    try:
        ...
        if len(soc) == packs:
            client.publish(config['mqtt_base_topic'] + "/pack_soc",str(round(sum(soc.values())/len(soc), 2)))
            if print_initial:
                print("Pack SOC: " + str(round(sum(soc.values())/len(soc)), 2) + " %")
        if len(soh) == packs:
